...
- Graphics card supporting OpenGL 4.0 or above (a fallback 3.3 version is available)
...
- Download the `mayonez-engine-<version>-<os>.zip` file for your operating system under the latest
  [release](https://github.com/SlavSquatSuperstar/mayonez-engine-2d/releases) on the GitHub website.
- Drag `run.sh` (Mac/Linux) or `run.bat`/`run.ps1` (Windows) from the extracted folder into a command line/terminal
  window and press Enter.
    - If you are blocked from running Terminal scripts, then run `chmod +x run.sh` in Terminal first.
    - If you are blocked from running PowerShell scripts, then run
      `Set-ExecutionPolicy -Scope CurrentUser RemoteSigned` in PowerShell first.
...
- The Gradle `packageMac`, `packageLinux`, and `packageWindows` tasks build and zip the project with the correct
...
- Java Version: [Eclipse Temurin 21+](https://adoptium.net/temurin/releases/?version=21) (FOSS) or
  [Oracle JDK 21+](https://www.oracle.com/java/technologies/downloads/#java21) (non-FOSS)
- Kotlin Version: [2.2+](https://kotlinlang.org/docs/getting-started.html)
- LWJGL Version: [3.3.6](https://www.lwjgl.org/customize)
- Test Framework: [JUnit 6](https://junit.org/junit5/docs/current/user-guide/)
- Build System: [Gradle 9.2+](https://gradle.org/install/)
- IDE: [Intellij IDEA 2025+](https://www.jetbrains.com/idea/download/) (recommended) or
