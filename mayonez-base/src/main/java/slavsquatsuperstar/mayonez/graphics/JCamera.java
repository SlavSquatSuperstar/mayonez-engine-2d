...
public final class JCamera extends Script implements Camera {
    ...
    public JCamera enableDragAndDrop(boolean enabled) {
        ...
        if (enabled && subject != null) {
            subject = null;
            Logger.debug("Scene: Subject following for %s has been disabled because freecam was enabled.", this, dragAndDrop);
        }
        return this;
    }
...
//            enableKeepInScene(subject.getComponent(KeepInScene.class) != null);
            if (dragAndDrop.isEnabled()) {
                ...
                Logger.debug("Scene: Freecam for %s has been disabled because a subject was set.", this, dragAndDrop);
            }
        }
        return this;
    }
