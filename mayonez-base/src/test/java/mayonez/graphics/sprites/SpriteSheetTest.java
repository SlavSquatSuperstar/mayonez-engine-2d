...
abstract class SpriteSheetTest {
    ...
    static Vec2[] SPRITE_ORIGINS, SPACED_SPRITE_ORIGINS;
    ...
    static void setup() {
        ...
        SPACED_SPRITE_ORIGINS = new Vec2[SpriteSheetTest.NUM_SPRITES];

        var rows = 2;
        var cols = SpriteSheetTest.NUM_SPRITES / rows;
        for (int y = 0; y < rows; y++) {
            for (int x = 0; x < cols; x++) {
                SPRITE_ORIGINS[y * cols + x] = new Vec2(x, y).mul(SPRITE_LENGTH);
                SPACED_SPRITE_ORIGINS[y * cols + x] = new Vec2(x, y)
